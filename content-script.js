...
const getVehicleId = ($img) => {
  ...
  const url = $img.getAttribute('src') === '/images/common/trans.gif' ? $img.getAttribute('data-src') : $img.getAttribute('src');
...
const checkUserSalesCount = async () => {
  const list = Array.from(document.querySelectorAll('#carResultListWrap li')).filter((el) => isNumeric(el.getAttribute('id')));

  const ids = Array.from(list).map((el) => {
    const vehicleId = getVehicleId(el.querySelector('img'))
    const registrantId = getRegistrantId(el.querySelector('.userMsg img'), el.getAttribute('id'))
  ...
  .filter(({vehicleId, registrantId}) => [vehicleId, registrantId].some((id) => id !== null))

  const testIds = ids.slice(0, 10);
  for (const {vehicleId, registrantId} of testIds) {
    const {currentlyOnSales, recentYearSales} = await getSalesCount({vehicleId, registrantId});
    ...
    if (recentYearSales > 200) {
      ...
      span.innerHTML = '↑ 200'
      document.getElementById(vehicleId).querySelector('.hotmark').appendChild(span);
    }
  }
...
const getSalesCount = async ({vehicleId, registrantId}) => {
  ...
  if (registrantId == null) {
    ...
    const { inspectionSource: {registrantId: _registrantId, updaterId, reservationId} } = productData
